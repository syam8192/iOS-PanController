...
class PanController: UIViewController {
	...
	convenience init(direction: ScrollDirection = .horizontal, loop: LoopMode = .loop) {
		self.init()
	...
	var scrollDirection: ScrollDirection = .horizontal {
		didSet {
			if scrollView?.subviews.count ?? 0 > 0 {
				containIndices = [-1, -1, -1]
				buildViews()
			}
		}
	}
	var arrowScrollOnlyEdge: Bool = false {
		didSet {
			if let scrollView = self.scrollView {
				scrollView.leftEdgeWidth = arrowScrollOnlyEdge ? 30 : 0
				scrollView.rightEdgeWidth = arrowScrollOnlyEdge ? 30 : 0
			}
		}
	}
	...
	func jumpTo(index: Int, animated: Bool) {
		...
		if let pageVC: UIViewController = pages[index] as? UIViewController {
			containers[i].subviews.forEach { $0.removeFromSuperview() }
			self.addChildViewController(pageVC)
			containers[i].addSubview(pageVC.view)
			addFillingParentConstraints(toView: pageVC.view)
			pageVC.didMove(toParentViewController: self)
			containers[i].setNeedsLayout()
			containers[i].layoutIfNeeded()
		}
		else if let pageView: UIView = pageView(at: index) {
			containers[i].subviews.forEach { $0.removeFromSuperview() }
	...
	fileprivate func updateContainer() {
		...
		if count > 2 {
			...
			case .loop:
				...
				containers[0].alpha = 1
				containers[2].alpha = 1
				containers[0].isHidden = false
				containers[2].isHidden = false
				prevScrollInset = 0
				nextScrollInset = 0
			case .none:
				containers[0].transform = CGAffineTransform.identity
		...
		for (i, newIndex) in newIndices.enumerated() {
			if newIndex >= 0 && newIndex != containIndices[i] {
				...
				if let pageVC: UIViewController = pages[newIndex] as? UIViewController {
				...
				else if let pageView: UIView = pageView(at: newIndex) {
					container.subviews.forEach { $0.removeFromSuperview() }
	...
	fileprivate func panScrollViewDidScroll(_ scrollView: UIScrollView, ignoreDelegate: Bool = false) {
		...
		if !isJumping {
			if scrollValue < scrollWidth {
				...
				if nextIndex < 0 {
					nextIndex = loopMode == .none ? 0 : count - 1
				...
				nextIndex = currentIndex + 1
				if nextIndex >= count {
					nextIndex = loopMode == .none ? count - 1 : 0
					willBeyondBound = true
				}
			}
		}
		transformer?.panController(self, views: containers, panningFromIndex: currentIndex, toIndex: nextIndex, progress: progress)
		
		// 境界.
		if count == 2 && currentIndex != nextIndex {
			...
			if rightContainer.subviews.count == 0 {
				if let nextView: UIView = pageView(at: nextIndex) {
		...
		if !ignoreDelegate {
			panDelegate?.panController(panController: self, didChangePanningFromIndex: recentIndex, toIndex: nextIndex, progress: progress)
		}
...
class PanScrollView: UIScrollView {
	...
	@IBInspectable var leftEdgeWidth: CGFloat = 0
	@IBInspectable var rightEdgeWidth: CGFloat = 0

	override func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
		if gestureRecognizer.numberOfTouches > 0 {
			let p = gestureRecognizer.location(ofTouch: 0, in: self).x - self.bounds.size.width
			if rightEdgeWidth > 0 && p < self.bounds.size.width - rightEdgeWidth {
				return false
			}
			if leftEdgeWidth > 0 && p < leftEdgeWidth {
				return false
			}
